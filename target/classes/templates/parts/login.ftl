<#macro login path isRegisterForm>
    <form action="${path}" method="post" class="form-signin">
        <h2 class="form-heading"><#if !isRegisterForm>Авторизация<#else>Регистрация</#if></h2>
        <h2 class="form-heading"></h2>

        <div class="form-group">
            <span>Авторизация</span>

            <input class="form-control mt-2" name="username" required type="text" class="form-control"
            ...
            <#if isRegisterForm><input class="form-control mt-2" name="email" required type="email"
                                       class="form-control mt-2" placeholder="some@some.ru" /></#if>
